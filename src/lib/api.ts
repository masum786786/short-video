...
export const api = {
  // Public Videos
  async getVideos(search?: string, refresh?: boolean): Promise<Video[]> {
    ...
    if (res.ok && res.data?.videos && Array.isArray(res.data.videos)) {
      const normalizedVideos: Video[] = res.data.videos.map((v: any) => ({
        ...v,
        _id: v._id || v.public_id || ('vid_' + Math.random().toString(36).substring(2)),
        title: v.title || v.public_id?.split('/')?.pop()?.replace(/[_-]/g, ' ') || 'Short Video',
        description: v.description || '',
        cloudinaryPublicId: v.cloudinaryPublicId || v.public_id || '',
        cloudinaryUrl: v.cloudinaryUrl || v.secure_url || '',
        ...
        updatedAt: v.updatedAt || v.created_at || new Date().toISOString(),
        public_id: v.public_id || v.cloudinaryPublicId,
        secure_url: v.secure_url || v.cloudinaryUrl,
        format: v.format || 'mp4',
      ...
      if (normalizedVideos.length > 0) {
        saveLocalVideos(normalizedVideos);
      }
