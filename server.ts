...
async function startServer() {
  ...
  // GET /api/videos - Return videos directly from Cloudinary search API (no MongoDB query)
  app.get('/api/videos', async (req, res) => {
    try {
      const search = (req.query.q as string || '').toLowerCase().trim();
      const forceRefresh = req.query.refresh === 'true';

      // Fetch videos from Cloudinary with 60-second in-memory rate-limiting cache
      let videos: any[] = [];
      try {
        videos = await getCachedCloudinaryVideos(forceRefresh);
      } catch (cloudErr) {
        console.warn('Cloudinary search failed, falling back to local store:', cloudErr);
      ...
      if (search) {
        videos = videos.filter(
          (v: any) =>
            ...
            v.public_id?.toLowerCase().includes(search)
        );
      }

      // Add 60-second caching headers to prevent exceeding Cloudinary Search API rate limits
  ...
  app.get('/api/videos/:id', async (req, res) => {
    try {
      ...
      try {
        const videos = await getCachedCloudinaryVideos();
        video = videos.find((v) => v.public_id === id || v._id === id || v.cloudinaryPublicId === id);
      } catch (_) {}

      if (!video) {
        video = await db.getVideoById(id);
      }
  ...
  app.get('/api/admin/videos', adminAuthMiddleware, async (req, res) => {
    try {
      ...
      const videos = await getCachedCloudinaryVideos(force);
      res.json({ videos });
    } catch (_) {
  ...
  app.post('/api/admin/videos/upload', adminAuthMiddleware, upload.single('video'), async (req, res) => {
    try {
      ...
      // Invalidate 60-second cache so new video appears right away
      invalidateVideosCache();
